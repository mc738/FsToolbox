...
module Expressions =
    ...
    type ExpressionOperatorToken =
        ...
        static member Deserialize(value: string) =
            ...
            let tryParse
                (operator: string)
                (handler: string -> string -> ExpressionOperatorToken)
                (result: ExpressionOperatorToken option)
                =
            ...
            |> tryParse "=~" (fun v op -> split op v |> ExpressionOperatorToken.RegexMatch)
    ...
    module Parsing =
            ...
            |> List.fold
                (fun (acc, lastSlice) m ->
                    ...
                    | TwoCharSymbol (c1, c2) when c1 = '&' && c2 = '&' ->
                        (acc
                         @ [ input.GetSlice(lastSlice, m.EndIndex - 2) |> Option.defaultValue "",
                             input.GetSlice(m.StartIndex, m.EndIndex) |> Option.defaultValue "" |> Some ],
                         m.EndIndex + 1)
                    | TwoCharSymbol (c1, c2) when c1 = '|' && c2 = '|' ->
                        (acc
                         @ [ input.GetSlice(lastSlice, m.EndIndex - 2) |> Option.defaultValue "",
                        ...
                        | Some o when o = "&&" ->
                            ...
                            | false ->
                                ExpressionStatement.And(
                                    ExpressionOperatorToken.Deserialize slice |> ExpressionStatement.Operator,
                                    acc
                                )
                        | Some o when o = "||" ->
                            match slice.StartsWith('(') with
        ...
        let parse input =
            ...
            with ex ->
                ExpressionStatementParseResult.Failure ex.Message
